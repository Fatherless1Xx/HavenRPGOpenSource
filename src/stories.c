...
#define DESTINY_FILE "../data/destiny.txt"
  ...
  _DOFUN(do_scene19location) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      argument = one_argument_nouncap(argument, argfour);
      char outbuf[MSL];
      snprintf(outbuf, sizeof(outbuf), "%d %.9000s %.9000s %.9000s", point, argtwo, argthree, argfour);
  ...
  _DOFUN(do_scene19specialroleone) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      argument = one_argument_nouncap(argument, argfour);
      char outbuf[MSL];
      snprintf(outbuf, sizeof(outbuf), "%.9000s %d %.9000s %.9000s", arg, point, argthree, argfour);
      free_string(ch->pcdata->ci_excludes[18]);
      ch->pcdata->ci_excludes[18] = str_dup(outbuf);
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
  ...
  _DOFUN(do_scene19specialroletwo) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      argument = one_argument_nouncap(argument, argfour);
      char outbuf[MSL];
      snprintf(outbuf, sizeof(outbuf), "%.9000s %.9000s %d %.9000s", arg, argtwo, point, argfour);
      free_string(ch->pcdata->ci_excludes[18]);
      ch->pcdata->ci_excludes[18] = str_dup(outbuf);
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
  ...
  _DOFUN(do_scene19conclusion) {
    ...
    if (ch->pcdata->ci_editing == 16) {
      ...
      argument = one_argument_nouncap(argument, argfour);
      char outbuf[MSL];
      snprintf(outbuf, sizeof(outbuf), "%.9000s %.9000s %.9000s %d", arg, argtwo, argthree, point);
      free_string(ch->pcdata->ci_excludes[18]);
      ch->pcdata->ci_excludes[18] = str_dup(outbuf);
      ch->pcdata->ci_editing = 0;
      send_to_char("Done.\n\r", ch);
...
*/
  ...
  char *narrative_pair_rewards(CHAR_DATA *ch, CHAR_DATA *victim) {
    ...
    char buf[MSL];
    snprintf(buf, sizeof(buf), "\n%.512s Narrative Rewards:\n%.12000s\n%.512s Narrative Rewards:\n%.12000s\r", ch->name, cbuf, victim->name, vbuf);
    return str_dup(buf);
  }
